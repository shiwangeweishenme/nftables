...
echo "支持 IPv4 / IPv6，批量添加规则，支持删除规则"
echo "=============================="
...
echo "2. 删除 IPv6 转发规则"
echo "3. 删除所有转发规则"
read -p "请选择要删除规则的类型 (1/2/3): " DELETE_OPTION

case $DELETE_OPTION in
  1)
    echo "🔧 删除 IPv4 转发规则："
    # 获取当前的 IPv4 转发规则并显示
    nft list ruleset | grep "tcp dport" | nl
    read -p "请输入要删除的规则编号: " DELETE_IPV4
    RULE_TO_DELETE=$(nft list ruleset | grep "tcp dport" | sed -n "${DELETE_IPV4}p")
    # 删除指定的 IPv4 转发规则
    nft delete rule ip forward prerouting handle $(echo "$RULE_TO_DELETE" | awk '{print $1}')
    ;;
  2)
    echo "🔧 删除 IPv6 转发规则："
    # 获取当前的 IPv6 转发规则并显示
    nft list ruleset | grep "ip6 dport" | nl
    read -p "请输入要删除的规则编号: " DELETE_IPV6
    RULE_TO_DELETE=$(nft list ruleset | grep "ip6 dport" | sed -n "${DELETE_IPV6}p")
    # 删除指定的 IPv6 转发规则
    nft delete rule ip6 forward6 prerouting handle $(echo "$RULE_TO_DELETE" | awk '{print $1}')
    ;;
  3)
    echo "删除所有转发规则..."
    nft flush ruleset
